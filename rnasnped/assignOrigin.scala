...
object assignOrigin{
  ...
  final val minExpr       =  3
  final val possibleYes   =  1
  final val possibleMaybe =  0
  final val possibleNo    = -1
  ...
  def main(args: Array[String]) = {
    ...
    val VCGroups   = VCF.groupVCs(VCF.readVCF(inputFile));
  ...
  def assignOriginToVCGroup(VCs: Array[VCF.VC], sTree: Array[sampleTree.treeNode]) = {
    ...
    if (!legalBase){
      ...
      val sampleAD = sTree.zipWithIndex.map( x => nodeInfoFeatCounts(x._2)("C%c" format alt).toString).mkString(",")
      val sampleCA = sTree.zipWithIndex.map( x => nodeInfoFeatCounts(x._2)("CA").toString).mkString(",")
      val sampleCC = sTree.zipWithIndex.map( x => nodeInfoFeatCounts(x._2)("CC").toString).mkString(",")
      val sampleCG = sTree.zipWithIndex.map( x => nodeInfoFeatCounts(x._2)("CG").toString).mkString(",")
      val sampleCT = sTree.zipWithIndex.map( x => nodeInfoFeatCounts(x._2)("CT").toString).mkString(",")
      val sampleDP = sTree.zipWithIndex.map( x => nodeInfoFeatCounts(x._2)("DP").toString).mkString(",")
      ...
      if(nodeSupport.last == 1) {
        ...
        VCF.VC(firstVC.contig,
               ...
               if(expr < 3) Array(SNP.lowExpression) else Array(VCF.pass),
               Map("SO" -> "1",
                   "SP" -> "1",
        ...
        if(origins.length == 0) {
          ...
          VCF.VC(firstVC.contig,
                 ...
                 Map("SO" -> origins.length.toString,
                     ...
                     "NI" -> origins.map( x => (x+1).toString).mkString(","),
                     "NP" -> formatNodePossible(sTree, nodePossible)))
        }
      }
    }
  ...
  def calcNodeFeatures(sTree: Array[sampleTree.treeNode], observedAltExprMap: Map[Int,Int], infoFeatCounts: Map[String,Map[Int,Int]]) = {
    ...
    var possible = collection.mutable.ArrayBuffer.fill(sTree.length)(possibleMaybe)
    ...
    sTree.foreach{
      case sampleTree.treeNode(id, name, children, isLeaf, height) =>
        ...
        possible(id) = if(isLeaf) { if(expr(id) > 0) possibleYes else if(expr(id) == 0) possibleMaybe else possibleNo }
                       else{ children.map(c => possible(c)).foldLeft(possibleYes)( (a,b) => math.min(a,b))  }
  ...
  def formatNodePossible(sTree: Array[sampleTree.treeNode], nodePossible: Array[Int]) = {
    nodePossible.map( x => if(x == possibleYes) 'y' else if(x == possibleMaybe) 'm' else 'n').mkString
  }
